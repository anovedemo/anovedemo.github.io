...
<code class="html" x-include="/admin-console/components/views/Company/get.html"></code>
The source of the above example is located at: /admin-console/components/views/Company/get.html 

#### Basic concept
```html
<draftsman-query alias="Companyget" x-include="/prepared-statements/views/Company/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/Company/get.txt) to an Alpine data object (Companyget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'Company'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.Companyget.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="Companyget" x-include="/form-definitions/Company/get.json"></div>
```
...
from the view updater indicating that view 'Company' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.company.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.company.get.key' refers to previously retrieved data using a query tag.
```html
<draftsman-notification message="updated" type="Company"
    @notification=" 
...
<code class="html" x-include="/admin-console/components/commands/ActivateUser.html"></code>

#### The basic concept
```html
<draftsman-mutation command="ActivateUser" x-include="/prepared-statements/commands/ActivateUser.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/ActivateUser.txt) as a command (ActivateUser) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="ActivateUser" x-include="/form-definitions/ActivateUser.json"></div>
```
...
```javascript
$store.mutation.send('ActivateUser',{all-form-data});
```
...
```html
<draftsman-trace command="ActivateUser" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'ActivateUser', data: {} });">
</draftsman-trace>
```
