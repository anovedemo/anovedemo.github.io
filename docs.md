...
<code class="html" x-include="/admin-console/components/views/ControlFramework/get.html"></code>
The source of the above example is located at: /admin-console/components/views/ControlFramework/get.html 

#### Basic concept
```html
<draftsman-query alias="ControlFrameworkget" x-include="/prepared-statements/views/ControlFramework/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/ControlFramework/get.txt) to an Alpine data object (ControlFrameworkget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'ControlFramework'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.ControlFrameworkget.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="ControlFrameworkget" x-include="/form-definitions/ControlFramework/get.json"></div>
```
...
from the view updater indicating that view 'ControlFramework' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.controlframework.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.controlframework.get.key' refers to previously retrieved data using a query tag.
```html
<draftsman-notification message="updated" type="ControlFramework"
    @notification=" 
...
<code class="html" x-include="/admin-console/components/commands/UpdateCompanyBusinessUnit.html"></code>

#### The basic concept
```html
<draftsman-mutation command="UpdateCompanyBusinessUnit" x-include="/prepared-statements/commands/UpdateCompanyBusinessUnit.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/UpdateCompanyBusinessUnit.txt) as a command (UpdateCompanyBusinessUnit) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="UpdateCompanyBusinessUnit" x-include="/form-definitions/UpdateCompanyBusinessUnit.json"></div>
```
...
```javascript
$store.mutation.send('UpdateCompanyBusinessUnit',{all-form-data});
```
...
```html
<draftsman-trace command="UpdateCompanyBusinessUnit" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'UpdateCompanyBusinessUnit', data: {} });">
</draftsman-trace>
```
