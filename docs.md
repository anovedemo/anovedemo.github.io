...
<code class="html" x-include="/admin-console/components/commands/AddIndustry.html"></code>

#### The basic concept
```html
<draftsman-mutation command="AddIndustry" x-include="/prepared-statements/commands/AddIndustry.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/AddIndustry.txt) as a command (AddIndustry) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="AddIndustry" x-include="/form-definitions/AddIndustry.json"></div>
```
...
```javascript
$store.mutation.send('AddIndustry',{all-form-data});
```
...
```html
<draftsman-trace command="AddIndustry" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'AddIndustry', data: {} });">
</draftsman-trace>
```
