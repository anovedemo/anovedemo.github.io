...
<code class="html" x-include="/admin-console/components/views/Industry/get.html"></code>
The source of the above example is located at: /admin-console/components/views/Industry/get.html 

#### Basic concept
```html
<draftsman-query alias="Industryget" x-include="/prepared-statements/views/Industry/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/Industry/get.txt) to an Alpine data object (Industryget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'Industry'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.Industryget.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="Industryget" x-include="/form-definitions/Industry/get.json"></div>
```
...
from the view updater indicating that view 'Industry' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.industry.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.industry.get.key' refers to previously retrieved data using a query tag.
```html
<draftsman-notification message="updated" type="Industry"
    @notification=" 
...
<code class="html" x-include="/admin-console/components/commands/UpdateBusinessRoleCompanyEmployee.html"></code>

#### The basic concept
```html
<draftsman-mutation command="UpdateBusinessRoleCompanyEmployee" x-include="/prepared-statements/commands/UpdateBusinessRoleCompanyEmployee.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/UpdateBusinessRoleCompanyEmployee.txt) as a command (UpdateBusinessRoleCompanyEmployee) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="UpdateBusinessRoleCompanyEmployee" x-include="/form-definitions/UpdateBusinessRoleCompanyEmployee.json"></div>
```
...
```javascript
$store.mutation.send('UpdateBusinessRoleCompanyEmployee',{all-form-data});
```
...
```html
<draftsman-trace command="UpdateBusinessRoleCompanyEmployee" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'UpdateBusinessRoleCompanyEmployee', data: {} });">
</draftsman-trace>
```
