...
<code class="html" x-include="/admin-console/components/views/ControlTemplate/get.html"></code>
The source of the above example is located at: /admin-console/components/views/ControlTemplate/get.html 

#### Basic concept
```html
<draftsman-query alias="ControlTemplateget" x-include="/prepared-statements/views/ControlTemplate/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/ControlTemplate/get.txt) to an Alpine data object (ControlTemplateget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'ControlTemplate'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.ControlTemplateget.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="ControlTemplateget" x-include="/form-definitions/ControlTemplate/get.json"></div>
```
...
from the view updater indicating that view 'ControlTemplate' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.controltemplate.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.controltemplate.get.key' refers to previously retrieved data using a query tag.
```html
<draftsman-notification message="updated" type="ControlTemplate"
    @notification=" 
...
<code class="html" x-include="/admin-console/components/commands/UpdateControlTemplate.html"></code>

#### The basic concept
```html
<draftsman-mutation command="UpdateControlTemplate" x-include="/prepared-statements/commands/UpdateControlTemplate.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/UpdateControlTemplate.txt) as a command (UpdateControlTemplate) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="UpdateControlTemplate" x-include="/form-definitions/UpdateControlTemplate.json"></div>
```
...
```javascript
$store.mutation.send('UpdateControlTemplate',{all-form-data});
```
...
```html
<draftsman-trace command="UpdateControlTemplate" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'UpdateControlTemplate', data: {} });">
</draftsman-trace>
```
