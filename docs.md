...
<code class="html" x-include="/admin-console/components/commands/UpdateRegion.html"></code>

#### The basic concept
```html
<draftsman-mutation command="UpdateRegion" x-include="/prepared-statements/commands/UpdateRegion.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/UpdateRegion.txt) as a command (UpdateRegion) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="UpdateRegion" x-include="/form-definitions/UpdateRegion.json"></div>
```
...
```javascript
$store.mutation.send('UpdateRegion',{all-form-data});
```
...
```html
<draftsman-trace command="UpdateRegion" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'UpdateRegion', data: {} });">
</draftsman-trace>
```
